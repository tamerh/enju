...
func newMCPHarness(t *testing.T, citizenName string) *mcpHarness {
	...
	cfg := mcphandlers.Config{
		...
		AuthToken:       firstActiveToken(t, ts.store, username),
...
// createTestProject delegates to the testServer helper. Every
// project gets origin (real remote or managed bare) — the dual-
// mode harness is gone after the no-remote state collapse.
...
func TestMCPRequestClarificationBotCaller(t *testing.T) {
	...
	if clarifySpawn.Subtype != string(store.CitizenKindAgent) {
		t.Errorf("bot caller: spawn subtype = %q, want %q (kind-derived trigger broken on the bot arm)",
			clarifySpawn.Subtype, string(store.CitizenKindAgent))
	}
...
func TestMCPCreateProject(t *testing.T) {
	...
	// Duplicate name — service.CreateProject surfaces the
	// coordinator's 409 directly as a tool error.
	dupText := h.callExpectError(t, "enju_create_project", map[string]any{
		"name": "Drug Target Discovery",
		"path": t.TempDir(),
	})
...
func TestMCPReviewRejectSpawnsRemediation(t *testing.T) {
	h := newMCPHarness(t, "RemediationSpawn")
	projectID := h.createTestProject()
...
func TestMCPAutoTriageFileAfterCompletedRun(t *testing.T) {
	h := newMCPHarness(t, "AutoTriagePostComplete")
	projectID := h.createTestProject()
...
func TestMCPListIterationsAfterReclaim(t *testing.T) {
	h := newMCPHarness(t, "ListIterReclaim")
	projectID := h.createTestProject()

...
type mcpHarness struct {
	*testServer
	...
	username string
	// useRemote controls whether h.createTestProject() returns a
	// remote-backed project (the historical default — bare repo
	// seeded under bareBaseDir) or a no-remote project (the
	// post-Option-B enju_create_project shape that real users
	// actually hit). eachRemoteMode toggles this between subtests.
	// Tests that genuinely need a remote — push verification,
	// scanner-from-remote, mcpBareResultMD readers — assert the
	// requirement via requireRemote(t, h) and skip the no-remote
	// subtest. Default is true so flat tests (not yet wrapped in
	// eachRemoteMode) keep their pre-existing behavior.
	useRemote bool
}
...
// newMCPHarnessMode is newMCPHarness with an explicit remote-mode
// override. Called by eachRemoteMode to spin up one harness per
// subtest under different remote configurations. Tests that opt
// into parameterization use eachRemoteMode; flat tests use
// newMCPHarness and inherit the remote-backed default.
func newMCPHarnessMode(t *testing.T, citizenName string, useRemote bool) *mcpHarness {
	t.Helper()
	h := newMCPHarness(t, citizenName)
	h.useRemote = useRemote
	return h
}

// createTestProject dispatches to the remote-backed or no-remote
// path based on h.useRemote. Tests call this without caring about
// the mode; eachRemoteMode runs the same body twice with the flag
// flipped.
func (h *mcpHarness) createTestProject() int64 {
	if h.useRemote {
		return h.testServer.createTestProject()
	}
	return h.testServer.createTestProjectNoRemote()
}

// eachRemoteMode runs body twice: once with a remote-backed
// project, once without. Each invocation spins a fresh harness
// (fresh testServer, fresh citizen, fresh workspace) under a
// distinct t.Run subtest so failures point at exactly which mode
// regressed.
//
// Tests that genuinely need a remote (push verification, scanner-
// from-remote, anything reading from bareRemotePath / using
// mcpBareResultMD) call requireRemote(t, h) at the top of the
// body — the no-remote subtest skips and only the with-remote
// subtest runs the body.
//
// Tests that are mode-agnostic (most state-machine, vote tally,
// review cascade, claim/submit transition tests) just use the
// helpers as-is and run under both subtests automatically.
//
// Cost: ~2x wall-clock for any wrapped test. The benefit is that
// every parameterized test verifies both modes on every CI run, so
// a regression in either path lights up as a specific subtest
// failure. Without this, the no-remote path was effectively
// untested and shipped a real bug
// (TestVoteSubmitCommitsWhenProjectHasNoRemote).
func eachRemoteMode(t *testing.T, citizenName string, body func(t *testing.T, h *mcpHarness)) {
	t.Helper()
	for _, m := range []struct {
		...
		m := m // capture
		t.Run(m.subtest, func(t *testing.T) {
			h := newMCPHarnessMode(t, citizenName, m.useRemote)
			body(t, h)
		})
	}
}

// requireRemote skips the current subtest when the harness is
// running in no-remote mode. Use at the top of test bodies that
// genuinely need a remote — push verification, bare-remote reads,
// scanner-from-remote behavior. The with-remote subtest runs
// normally; the no-remote subtest skips with a clear reason.
func requireRemote(t *testing.T, h *mcpHarness) {
	t.Helper()
	if !h.useRemote {
		t.Skip("test requires a remote-backed project (push / bare-read / scanner-from-remote)")
	}
}
...
func TestMCPHarnessSmokesEnjuMyProfile(t *testing.T) {
	eachRemoteMode(t, "Harness Smoke", func(t *testing.T, h *mcpHarness) {
...
func TestMCPHappyPathFullCycle(t *testing.T) {
	eachRemoteMode(t, "Happy Path", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPSubmitResultReviewPreValidationBlocksGit(t *testing.T) {
	eachRemoteMode(t, "Review PreValidator", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPRunStatusRendersTreeWithMixedStates(t *testing.T) {
	eachRemoteMode(t, "Run Status Formatter", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		yamlBody, err := readFixture("review.yaml")
		if err != nil {
			t.Fatalf("read fixture: %v", err)
...
func TestMCPClaimRendersPromptBlock(t *testing.T) {
	eachRemoteMode(t, "Claim Formatter", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		yamlBody, err := readFixture("simple-no-deps.yaml")
		if err != nil {
			t.Fatalf("read fixture: %v", err)
...
func TestMCPMultiCitizenSubmitRoutesPerCitizen(t *testing.T) {
	eachRemoteMode(t, "Drafter", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
...
func TestMCPClaimReviewTaskInlinesTargetContent(t *testing.T) {
	eachRemoteMode(t, "Review Claimer", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimSurfacesRequestChangesFeedback(t *testing.T) {
	eachRemoteMode(
		...
		t, "Drafter", func(t *testing.T, h *mcpHarness) {
...
func TestMCPSubmitResultVotePreValidationBlocksGit(t *testing.T) {
	eachRemoteMode(t, "Vote PreValidator", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPBranchingWithInferredDeps(t *testing.T) {
	eachRemoteMode(t, "Branching Alice", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
...
func TestMCPForEachExpansion(t *testing.T) {
	eachRemoteMode(t, "ForEach", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPMultipleCitizensCollaborate(t *testing.T) {
	eachRemoteMode(t, "Trio Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPCitizenDashboard(t *testing.T) {
	eachRemoteMode(t, "Dashboard", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPActionField(t *testing.T) {
	eachRemoteMode(t, "Action Field", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "branching.yaml")
...
func TestMCPNamedOutputs(t *testing.T) {
	eachRemoteMode(t, "Named Outputs", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPCreateProject(t *testing.T) {
	eachRemoteMode(t, "Create Project", func
...
func TestMCPRunInProject(t *testing.T) {
	eachRemoteMode(t, "Run-In-Project", func
...
func TestMCPEnvironmentRequirements(t *testing.T) {
	eachRemoteMode(t, "Requirements", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPMultiFileOutputs(t *testing.T) {
	eachRemoteMode(t, "Multi File Outputs", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPUpdateProfile(t *testing.T) {
	eachRemoteMode(t, "Profile Owner", func(t *testing.T, h *mcpHarness) {
...
func TestMCPTaskInputsSurfacesMissingArtifacts(t *testing.T) {
	eachRemoteMode(t, "Missing Artifact", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPContributionEventsRecorded(t *testing.T) {
	eachRemoteMode(t, "Contributor", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPArtifactWriteAndRead(t *testing.T) {
	eachRemoteMode(t, "Artifact Writer", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPArtifactWriteRejectedForUndeclaredPath(t *testing.T) {
	eachRemoteMode(t, "Sneaky Writer", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "artifacts.yaml")
...
func TestMCPArtifactListAndGet(t *testing.T) {
	eachRemoteMode(t, "Artifact Lister", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "artifacts.yaml")
...
func TestMCPArtifactFieldsInTaskResponse(t *testing.T) {
	eachRemoteMode(t, "Artifact Fields", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "artifacts.yaml")
...
func TestMCPArtifactPermissiveWrites(t *testing.T) {
	eachRemoteMode(t, "Permissive", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPAccessControlDefaultOpen(t *testing.T) {
	eachRemoteMode(t, "Access Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlAssignToAllowsListedCitizen(t *testing.T) {
	eachRemoteMode(t, "AssignTo Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlAssignToListRejectsWithBothNames(t *testing.T) {
	eachRemoteMode(t, "ListAssign Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlRequireRoleAllows(t *testing.T) {
	eachRemoteMode(t, "RoleAllowed Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlRequireRoleRejects(t *testing.T) {
	eachRemoteMode(t, "RoleRejected Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlBothRestrictionsMustMatch(t *testing.T) {
	eachRemoteMode(t, "BothCheck Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPAccessControlScalarAssignTo(t *testing.T) {
	eachRemoteMode(t, "Scalar Alice", func(t *testing.T, h *mcpHarness) {
...
func TestMCPInvalidateCascadesDownstream(t *testing.T) {
	eachRemoteMode(t, "Cascade Alice", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "branching.yaml")
...
func TestMCPInvalidateAllowsReclaimAndProgression(t *testing.T) {
	eachRemoteMode(t, "Reclaim Alice", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "branching.yaml")
...
func TestMCPInvalidateRollsBackArtifactToPriorWriter(t *testing.T) {
	eachRemoteMode(t, "Rollback Alice", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPInvalidateFirstWriterDeletesArtifact(t *testing.T) {
	eachRemoteMode(t, "FirstWriter", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPInvalidateWalkerSkipsPreviouslyInvalidatedWriter(t *testing.T) {
	eachRemoteMode(t, "Walker Alice", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPInvalidateRejectsNonAcceptedTarget(t *testing.T) {
	eachRemoteMode(t, "InvRejecter", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "branching.yaml")
...
func TestMCPReviewMetadataAuditTrail(t *testing.T) {
	eachRemoteMode(t, "Audit Drafter", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
...
func TestMCPReviewRejectMetadataCarriesVerdict(t *testing.T) {
	eachRemoteMode(t, "Reject-Audit Drafter", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
...
func TestMCPVoteSubmitMaterializesMetadata(t *testing.T) {
	eachRemoteMode(t, "VoteMaterialize", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteForEachSubmitMaterializesMetadata(t *testing.T) {
	eachRemoteMode(t, "VoteForEach", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteSubmitOptionOnlyNoContent(t *testing.T) {
	eachRemoteMode(t, "VoteOptionOnly", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
`
		h.mcpCreateRunInline(t, projectID, yaml)
		h.mcpClaimOK(t, "pick")
		proj, _ := h.enjugit.ForProject(projectID, h.remoteFor(projectID))
		preHead, _, _ := proj.Head()
...
// TestMCPVoteSubmitMatchesTesterReport reproduces the tester's
// exact scenario: for_each over item_id=[i01, i02], task id "vote",
// distinct models per submit. The tester reports no commits and no
// vote/item_id=i01/ directory; this test verifies whether the
// shape they describe materializes (which it should — earlier
// repros confirm vote submits commit + write metadata).
//
// Either this passes (suggesting the tester's environment differs:
// stale binary, looking at the wrong git ref, etc.) or fails
// (revealing a code path the earlier tests missed).
func TestMCPVoteSubmitMatchesTesterReport(t *testing.T) {
	eachRemoteMode(t, "TesterRepro", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
`
		...
		h.callOK(t, "enju_submit_result", map[string]any{
			"task_id": h.taskID("i01:vote"),
			"option":  "good",
			"model":   "gemini-3-1-pro-preview",
		})

		// Submit i02 with a different model.
		h.mcpClaimOK(t, "i02:vote")
		h.callOK(t, "enju_submit_result", map[string]any{
			"task_id": h.taskID("i02:vote"),
			"option":  "bad",
			"model":   "gpt-5-1",
		})
		...
		// Both metadata.json files at the expected path.
		for _, item := range []string{"i01", "i02"} {
			path := filepath.Join(proj.WorkDir(), h.runDir(1), "vote", "item_id="+item, "metadata.json")
			if _, err := os.Stat(path); err != nil {
				t.Errorf("metadata.json missing for %s at %s: %v", item, path, err)
			}
		}
	})
}

// TestMCPVoteSubmitCommitsWithoutRemote is the regression test for
// the silent-no-commit bug a real tester surfaced. Setup mirrors
// what enju_create_project produces with no remote_url= flag (the
// post-Option-B default — no shadow bare gets auto-created).
//
// Pre-fix, useFatClient returned false for projects with no remote
// AND no init-registered external dir, so vote / review / answer
// submits silently went to the legacy POST path: coordinator
// recorded state=accepted + result_path but no commit landed and
// no metadata.json appeared on disk. Compute tasks happened to work
// because they bypass useFatClient via compute.Run →
// Project.SubmitTaskResult.
//
// Post-fix, useFatClient returns true whenever workspace is
// configured. Local clones receive the commit even without a
// remote (push is a no-op).
//
// Existing TestMCPVote* tests use createTestProject which always
// sets remote_url, so they exercised only the working path. This
// test plugs that gap.
func TestMCPVoteSubmitCommitsWithoutRemote(t *testing.T) {
	eachRemoteMode(t, "NoRemoteVote", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProjectNoRemote()

		yaml := `name: "no-remote-vote"
version: 1
tasks:
  - id: pick
    action: vote
...
`
		...
		proj, err := h.enjugit.ForProject(projectID, "")
		if err != nil {
		...
		// metadata.json materializes at the per-task path.
		metadataPath := filepath.Join(proj.WorkDir(), h.runDir(1), "pick", "metadata.json")
		if _, err := os.Stat(metadataPath); err != nil {
			t.Fatalf("metadata.json missing at %s: %v", metadataPath, err)
		}
...
func TestMCPVotePureDecisionNoSkipCascade(t *testing.T) {
	eachRemoteMode(t, "PureVote", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteInvalidationResetsSkipped(t *testing.T) {
	eachRemoteMode(t, "VoteReset", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteMultiCitizenCollectsThenResolves(t *testing.T) {
	eachRemoteMode(t, "QuorumA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPVoteMultiCitizenRejectDoubleClaim(t *testing.T) {
	eachRemoteMode(t, "Double", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteMultiCitizenCapAtCitizensLimit(t *testing.T) {
	eachRemoteMode(t, "CapA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPMultiReviewerAnyRejectKills(t *testing.T) {
	eachRemoteMode(t, "AnyRejectA", func(t *testing.T, h *mcpHarness) {
		...
		_ = charlie
	}) // never submitted — task was already resolved.
}
...
func TestMCPMultiReviewerHardRejectOverridesSoft(t *testing.T) {
	eachRemoteMode(t, "HardA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPRejectCascadesFailArtifactAndDownstream(t *testing.T) {
	eachRemoteMode(t, "RCReviewer", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRejectCascadesDownstreamOfReviewGate(t *testing.T) {
	eachRemoteMode(t, "RCGate", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRequestChangesCascadesDownstreamOfReviewGate(t *testing.T) {
	eachRemoteMode(t, "RCGateRC", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRejectCascadesGateInForEachInstance(t *testing.T) {
	eachRemoteMode(t, "RCGateFE", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRejectCascadeSparesCrossIterationAggregator(t *testing.T) {
	eachRemoteMode(t, "ForEachAgg", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteActivatesSkipCascadeInForEach(t *testing.T) {
	eachRemoteMode(t, "VoteAct", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPReadsArtifactsNoDuplicatesAcrossSubstitution(t *testing.T) {
	eachRemoteMode(t, "ReadsDedup", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteActivatesSkipCascadeSparesCrossIterationAggregator(t *testing.T) {
	eachRemoteMode(t, "VoteAggActivates", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPVoteActivatesSingletonActivateeSurvives(t *testing.T) {
	eachRemoteMode(t, "VoteSingletonAct", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimLeanMode(t *testing.T) {
	eachRemoteMode(t, "LeanClaim", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPSubmitResultsBatchHappyPath(t *testing.T) {
	eachRemoteMode(t, "BatchRev", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPSubmitResultsBatchRejectsIntraBatchDependency(t *testing.T) {
	eachRemoteMode(t, "BatchDep", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPSubmitResultsBatchScopeChecks(t *testing.T) {
	eachRemoteMode(t, "BatchScope", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPSubmitResultsBatchCoalescesPushes(t *testing.T) {
	eachRemoteMode(t, "BatchPush", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPSubmitResultsBatchRollsBackOrphanCommits(t *testing.T) {
	eachRemoteMode(t, "BatchRollback", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimReadyMatchingHappyPath(t *testing.T) {
	eachRemoteMode(t, "ClaimSelect", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimReadyMatchingActionFilter(t *testing.T) {
	eachRemoteMode(t, "ClaimFilter", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimReadyMatchingLimitCap(t *testing.T) {
	eachRemoteMode(t, "ClaimCap", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimReadyMatchingDedupAlreadyClaimed(t *testing.T) {
	eachRemoteMode(t, "ClaimDedup", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRunStatusMermaidFormat(t *testing.T) {
	eachRemoteMode(t, "MermaidA", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase1Parking(t *testing.T) {
	eachRemoteMode(t, "ParkA", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2IdenticalList(t *testing.T) {
	eachRemoteMode(t, "RematIdentical", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2KeyRemoved(t *testing.T) {
	eachRemoteMode(t, "RematRemove", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2KeyAdded(t *testing.T) {
	eachRemoteMode(t, "RematAdd", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2MixedDiff(t *testing.T) {
	eachRemoteMode(t, "RematMixed", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2SingletonReopensOnDepChange(t *testing.T) {
	eachRemoteMode(t, "RematSingleton", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2SingletonPreservedOnIdenticalDeps(t *testing.T) {
	eachRemoteMode(t, "RematSingletonKeep", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPForEachArtifactPathSubstitution(t *testing.T) {
	eachRemoteMode(t, "ArtifactSubst", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachArtifactPairedDeps(t *testing.T) {
	eachRemoteMode(t, "DynArtPair", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPComputeScriptLog(t *testing.T) {
	eachRemoteMode(t, "ScriptLog", func(t *testing.T, h *mcpHarness) {
		// mcpBareResultMD reads from the bare remote; with no
		// remote configured, the read has nowhere to go.
		// The same compute path works in both modes — this test
		// just asserts via a remote-only helper.
		requireRemote(t, h)
...
func TestMCPForEachSlashKeyRoutable(t *testing.T) {
	eachRemoteMode(t, "SlashKey", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPExportRunEvents(t *testing.T) {
	eachRemoteMode(t, "EventsExportA", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeEnvVarsParams(t *testing.T) {
	eachRemoteMode(t, "ComputeEnvA", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeTaskEnvBlock(t *testing.T) {
	eachRemoteMode(t, "ComputeTaskEnv", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeTaskEnvParamSubstitution(t *testing.T) {
	eachRemoteMode(t, "ComputeTaskEnvParams", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPParamDefaultAppliesWhenUnsupplied(t *testing.T) {
	eachRemoteMode(t, "ParamDefault", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPParamSuppliedOverridesDefault(t *testing.T) {
	eachRemoteMode(t, "ParamOverride", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeTaskEnvRejectsReservedPrefix(t *testing.T) {
	eachRemoteMode(t, "ComputeTaskEnvReserved", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeTaskEnvRejectedOnNonCompute(t *testing.T) {
	eachRemoteMode(t, "ComputeTaskEnvNonCompute", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeContextJSON(t *testing.T) {
	eachRemoteMode(t, "ComputeCtxJSON", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPTemplateBundleSnapshotAndExec(t *testing.T) {
	eachRemoteMode(t, "TemplateBundle", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPComputeWritesArtifactsRegisters(t *testing.T) {
	eachRemoteMode(t, "ComputeWrites", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPComputeUntrackedArtifactStaysOutOfCommit(t *testing.T) {
	eachRemoteMode(t, "ComputeUntracked", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimRefusesMissingUntrackedRead(t *testing.T) {
	eachRemoteMode(t, "ClaimUntrackedMissing", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimAllowsPresentUntrackedRead(t *testing.T) {
	eachRemoteMode(t, "ClaimUntrackedPresent", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPListUntrackedArtifactsReportsPresence(t *testing.T) {
	eachRemoteMode(t, "ListUntracked", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPComputeContainerMissingDockerFriendlyError(t *testing.T) {
	eachRemoteMode(
		...
		t, "ContainerNoDocker", func(t *testing.T, h *mcpHarness) {
			projectID := h.createTestProject()
...
func TestMCPListUntrackedArtifactsEmpty(t *testing.T) {
	eachRemoteMode(t, "ListUntrackedEmpty", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPComputeLintWarnsOnHiddenDeps(t *testing.T) {
	eachRemoteMode(t, "ComputeLintA", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase5UX(t *testing.T) {
	eachRemoteMode(t, "RematUX", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPPartialRematPhase2ReviewVerdictPreserved(t *testing.T) {
	eachRemoteMode(t, "RematReview", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPMermaidFanOutFromDynamicForEach(t *testing.T) {
	eachRemoteMode(t, "FanOutA", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPExportDiagram(t *testing.T) {
	eachRemoteMode(t, "ExportDiagramA", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPVoteResponsesTemplate(t *testing.T) {
	eachRemoteMode(t, "VResponsesA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPVoteLateSubmitAfterResolve(t *testing.T) {
	eachRemoteMode(t, "LateVoteA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPVoteDefaultQuorumMatchesCitizens(t *testing.T) {
	eachRemoteMode(t, "DefQuorumA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPReviewLateSubmitAfterResolve(t *testing.T) {
	eachRemoteMode(t, "LateReviewA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPReviewResponsesTemplate(t *testing.T) {
	eachRemoteMode(t, "RRespA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPVoteDeadlineLazyResolve(t *testing.T) {
	eachRemoteMode(t, "DeadlineA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPCreateRunWithParams(t *testing.T) {
	eachRemoteMode(t, "ParamsOK", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPCreateRunWithParamsMissingRequired(t *testing.T) {
	eachRemoteMode(t, "ParamsMissing", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachMaterializes(t *testing.T) {
	eachRemoteMode(t, "DynamicA", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachPerInstanceReviewChain(t *testing.T) {
	eachRemoteMode(t, "DynReview", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachInvalidationCascade(t *testing.T) {
	eachRemoteMode(t, "DynInval", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachPerInstancePromptResolves(t *testing.T) {
	eachRemoteMode(t, "PerInstancePrompt", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachSingletonFanInAggregates(t *testing.T) {
	eachRemoteMode(t, "SingletonFanIn", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachPerInstanceReviewCascades(t *testing.T) {
	eachRemoteMode(t, "CascadeDrafter", func(t *testing.T, h *mcpHarness) {
...
func TestMCPDynamicForEachPerInstanceReviewShowsTargetContent(t *testing.T) {
	eachRemoteMode(t, "InlineDrafter", func(t *testing.T, h *mcpHarness) {
...
func TestMCPTemplateParamInAssignTo(t *testing.T) {
	eachRemoteMode(t, "signoff-person", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPTemplateParamInRunLevelForEach(t *testing.T) {
	eachRemoteMode(t, "RunLevelParamForEach", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPTemplateParamInForEachList(t *testing.T) {
	eachRemoteMode(t, "ParamForEach", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPClaimResolvesSkippedUpstream(t *testing.T) {
	eachRemoteMode(t, "SkippedResolve", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPRunStatusShowsSkippedSeparately(t *testing.T) {
	eachRemoteMode(t, "SkippedCountFix", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
		h.mcpCreateRunFromFixture(t, projectID, "vote-gate.yaml")
...
func TestMCPDynamicForEachVoteWinningOptionFanIn(t *testing.T) {
	eachRemoteMode(t, "WinningOptFanIn", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachVoteResponsesFanIn(t *testing.T) {
	eachRemoteMode(t, "RespFanInA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPSubmitPreservesManualUserCommits(t *testing.T) {
	eachRemoteMode(t, "Polite Guest", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachInvalidateSingleInstance(t *testing.T) {
	eachRemoteMode(t, "InvalSingleInst", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachComputeAction(t *testing.T) {
	eachRemoteMode(t, "ComputeForEach", func(t *testing.T, h *mcpHarness) {
		requireRemote(t, h)
		projectID := h.createTestProject()
...
func TestMCPDynamicForEachPerInstanceMultiVoter(t *testing.T) {
	eachRemoteMode(t, "MultiVoteA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPDynamicForEachPerInstanceMultiReviewer(t *testing.T) {
	eachRemoteMode(t, "MultiRevA", func(t *testing.T, h *mcpHarness) {
...
func TestMCPDynamicForEachPerInstanceRevisionContext(t *testing.T) {
	eachRemoteMode(t, "RevisionCtxDrafter", func(t *testing.T, h *mcpHarness) {
...
func TestMCPDynamicForEachInvalidationParks(t *testing.T) {
	eachRemoteMode(t, "DynPark", func(t *testing.T, h *mcpHarness) {
		projectID := h.createTestProject()
...
func TestMCPReviewImplicitGating(t *testing.T) {
	eachRemoteMode(t, "Implicit Drafter", func(t *testing.T, h *mcpHarness) {

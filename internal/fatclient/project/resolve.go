...
// citizen upstream task. Content lives in git only; the resolver
// reads it from the per-citizen subdirectory at CommitSHA.
//
// PathUsername is the real submitter username used to construct
// the file path (`{parent_result_dir}/citizen-{path_username}/
// result.md`). Username is what gets shown in the rendered
// `{{task.responses}}` block — same as PathUsername in the
// normal case, replaced by `citizen-N` when the upstream task
// has anonymize: true.
...
func (p *Clone) readResultForTemplate(dep DependencyRef) (map[string]interface{}, error) {
	...
	if len(dep.Responses) > 0 {
		...
		for _, r := range dep.Responses {
			// Content lives in git only — coord no longer ships
			// it inline (per ARCHITECTURE.md #3). Read each
			// citizen's result.md from `{parent_result_dir}/
			// citizen-{path_username}/result.md` at the
			// citizen's own commit_sha. Best-effort: a missing
			// commit or file leaves content empty rather than
			// failing the resolve.
			var content string
			if r.CommitSHA != "" && r.PathUsername != "" {
				citizenPath := filepath.Join(dep.ResultPath, "citizen-"+r.PathUsername, "result.md")
				if data, ok, _ := p.readAt(r.CommitSHA, citizenPath); ok {
					content = string(data)
				}
			}

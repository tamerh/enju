...
func TestSharedArtifactPathNumericFallback(t *testing.T) {
	// Empty project name → numeric-id directory.
...
func TestEnsureSharedSymlinkIdempotent(t *testing.T) {
	...
	// Second call — must not error, target unchanged.
	if err := EnsureSharedSymlink("out/x", workDir, 1, "demo", "main", "out/x"); err != nil {
		t.Fatal(err)
	}
...
func TestEnsureSharedSymlinkReplacesStale(t *testing.T) {
	...
	// Seed a symlink pointing at the wrong place.
...
func TestEnsureSharedSymlinkPreservesRegularFile(t *testing.T) {
	...
	// Seed a real file — citizen has local bytes.
	wsPath := filepath.Join(workDir, "out/x")
	_ = os.MkdirAll(filepath.Dir(wsPath), 0755)
	...
	// File must still be a regular file (mode bits say no
	// symlink) with the original content.
	fi, err := os.Lstat(wsPath)
	if err != nil {
		t.Fatal(err)
	}
...
// TestEnsureSharedSymlinkCreatesSharedParent — the helper
// is responsible for making the shared-side parent directory
// so the producer's subsequent write (through the symlink)
// succeeds.

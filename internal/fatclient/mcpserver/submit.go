...
func (c *apiClient) submitOneForBatch(ctx context.Context, e batchSubmission, meta *taskMeta) batchEntryResult {
	...
	if !c.useFatClient(meta) {
		// Same git-required contract as handleSubmitResult.
		return batchEntryResult{TaskID: e.TaskID, Status: "error", Message: "submit requires a local workspace; run via `enju mcp`"}
	}

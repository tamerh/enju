...
func (c *Clone) CheckoutBranchFrom(branch, baseBranch, defaultBranch string) error {
	...
	// Checkout the new branch's tree. v6 simplification: no
	// preserve dance — go-git v6's Force checkout natively
	// preserves untracked files (PR #1903), so user-authored
	// scratch / gitignored artifacts survive the branch swap
	// automatically. checkoutRef (branch.go) carries the v6
	// ordering workaround so tracked-on-old-not-on-new files
...
// (IsAncestor + commitHasAncestor live in branch.go in this
// package — they were moved during the v6 port to avoid
// circular helper duplication. See branch.go for the impls.)

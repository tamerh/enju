...
// Read-only: does not acquire the project lock.
//
// v6 note: the v5 implementation also returned StateMidCheckout
// when a leftover preserve dir was on disk. v6 doesn't need
// preserve.go (PR #1903 made go-git's Force checkout preserve
// untracked natively), so that branch is gone here. State no
// longer returns StateMidCheckout.

...
// CommitRunTemplateSnapshot. Materializes a run's template
// snapshot from git history into an on-disk directory, so
// compute.Run can execute its script as a normal subprocess
// (subprocesses can't read from .git/objects/ — they need
// real files).
//
// Critical for concurrent create_run support: each
// execute_run call materializes ITS OWN run-branch's
// snapshot into a per-task path, decoupled from the shared
// operator worktree. Without this, N parallel runs serialize
// through the worktree (one branch checked out at a time) and
// the snapshot files for earlier runs get overwritten by
// later checkouts. The plumbing write path
// (CommitArbitraryFilesPlumbing) lands every run's snapshot
// in its own branch's history; this materializer pulls them
// back out at execute time.

...
`)
	...
	if parsed.Problem.Name != "Test Problem" {
		t.Fatalf("expected name 'Test Problem', got %q", parsed.Problem.Name)
	}
...
func TestParseAssistedMode(t *testing.T) {
	yamlData := []byte(`
name: "Deliberation"
version: 1
tasks:
...
`)
	...
	if tasks[0].Mode != "assisted" {
		t.Fatalf("expected assisted mode, got %q", tasks[0].Mode)
	}
...
func TestValidationErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name: "missing name",
			yaml: `tasks: [{id: a, type: llm_prompt, prompt: "x"}]`,
		...
		},
		{
			...
			yaml: `name: "test"
tasks: [{type: llm_prompt, prompt: "x"}]`,
		...
		},
		{
			...
			yaml: `name: "test"
tasks:
  - {id: a, type: llm_prompt, prompt: "x"}
  - {id: a, type: llm_prompt, prompt: "y"}`,
			err: "duplicate task ID",
		},
		{
			name: "invalid type",
			yaml: `name: "test"
tasks: [{id: a, type: invalid, prompt: "x"}]`,
			err: "invalid type",
		},
		{
			name: "missing prompt",
			yaml: `name: "test"
tasks: [{id: a, type: llm_prompt}]`,
			err: "prompt is required",
		},
		{
			name: "missing script",
			yaml: `name: "test"
tasks: [{id: a, type: script}]`,
		...
		},
		{
			...
			yaml: `name: "test"
tasks:
  - {id: a, type: llm_prompt, prompt: "x", depends_on: [nonexistent]}`,
			err: "does not exist",
		},
		{
			name: "cycle",
			yaml: `name: "test"
tasks:
  - {id: a, type: llm_prompt, prompt: "x", depends_on: [b]}
  - {id: b, type: llm_prompt, prompt: "y", depends_on: [a]}`,
			err: "cycle",
		},
	}

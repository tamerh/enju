			...
			if depTask.Citizens > 1 {
				if submissions, err := e.store.ListVoteSubmissions(depTask.ID); err == nil && len(submissions) > 0 {
					...
					for idx, sub := range submissions {
						realUsername := e.citizenUsername(sub.CitizenID)
						displayUsername := realUsername
						if depTask.Anonymize {
							displayUsername = fmt.Sprintf("citizen-%d", idx+1)
						}
						// Per-citizen submission lives at:
						//   {parent_result_dir}/citizen-{real_username}/result.md
						// Pass commit_sha so the fat-client can read
						// it from git directly. Coordinator no longer
						// returns prose content; that lives only in
						// the per-citizen commit.
						//
						// Anonymize note: real_username is in the path
						// — anonymize hides display names but doesn't
						// hide identity from anyone who can read the
						// downstream's local clone. Documented in
						// TODO under "metadata privacy gap"; fully
						// hardening anonymize is a separate redesign.
						perCitizen = append(perCitizen, map[string]interface{}{
							"username":      displayUsername,
							"real_username": realUsername,
							"option":        sub.Option,
							"commit_sha":    sub.CommitSHA,
						})
					}

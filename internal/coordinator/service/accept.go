...
func (c *Coordinator) AcceptComputeTaskCore(
	...
	if actions != nil && len(actions.ArtifactMutations) > 0 {
		if _, err := c.Store.ApplyPlan(store.Plan{
			...
			// submission has already been accepted at the task
			// level, and an artifact-index write failure is
			// recoverable (the next submit or invalidation
			// rebuilds the row). Matches pre-refactor sync
			// behavior.

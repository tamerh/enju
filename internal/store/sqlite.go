...
func (s *Store) GetTask(id string) (*TaskRecord, error) {
	...
	err := s.db.QueryRow(
		...
		&anonymizeInt, &t.Visibility, &t.FailReason, &t.SkipReason, &t.ParkedFromState, &t.Env,
		&t.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
